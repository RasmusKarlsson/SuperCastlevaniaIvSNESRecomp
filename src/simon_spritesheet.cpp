...
bool Load(){int w=0,h=0,n=0;uint8_t*px=stbi_load(kFilename,&w,&h,&n,4);if(!px||w!=(int)kWidth||h!=(int)kHeight){if(px)stbi_image_free(px);std::fprintf(stderr,"[Simon sheet] %s must remain %ux%u RGBA\n",kFilename,kWidth,kHeight);return false;}edited.fill(0);for(unsigned ti=0;ti<kTiles;++ti){unsigned ox=(ti%kColumns)*8,oy=(ti/kColumns)*8;const uint8_t*source=&original[ti*kTileBytes];uint8_t*t=&edited[ti*kTileBytes];for(unsigned y=0;y<8;++y)for(unsigned x=0;x<8;++x){const uint8_t*in=&px[((oy+y)*kWidth+ox+x)*4];uint8_t original_index=GetPixel(source,x,y),expected[4];Color(base_palette[original_index],expected);if(!original_index)expected[3]=0;uint8_t index;if(std::memcmp(in,expected,4)==0)index=original_index;else index=in[3]<128?0:(uint8_t)Nearest(in,base_palette.data());SetPixel(t,x,y,index);}}stbi_image_free(px);size_t changed=0;for(size_t i=0;i<kSheetBytes;++i)changed+=edited[i]!=original[i];std::fprintf(stderr,"[Simon sheet] loaded %s; watching for edits (%zu source bytes changed)\n",kFilename,changed);return true;}
