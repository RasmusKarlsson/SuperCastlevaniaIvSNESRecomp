#!/usr/bin/env bash
# Regenerate src/gen/*.c for Super Castlevania IV from a verified ROM.
#
...
if [ -z "$ROM" ]; then
  for cand in "org - Copy.sfc" "supercastlevaniaiv.sfc" "supercastlevaniaiv.smc"; do
    if [ -f "$cand" ]; then ROM="$cand"; break; fi
  done
fi
if [ -z "$ROM" ] || [ ! -f "$ROM" ]; then
  echo "regen.sh: no ROM found." >&2
  echo "          Pass --rom /path/to/org - Copy.sfc, set SNESRECOMP_ROM, or put" >&2
  echo "          it at the repo root. You must legally own a copy of" >&2
  echo "          Super Castlevania IV." >&2
  exit 1
fi
